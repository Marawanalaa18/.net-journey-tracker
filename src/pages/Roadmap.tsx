...
const Roadmap = () => {
  const { user, stages, lessons } = useApp();
      ...
      <div className="relative mx-auto max-w-3xl">
        ...
        <div className="space-y-6">
          {stages.map((stage, i) => {
            ...
            const allDone = stageLessons.length > 0 && stageCompleted === stageLessons.length;
            const pct = stageLessons.length > 0 ? Math.round((stageCompleted / stageLessons.length) * 100) : 0;

            return (
              <div key={stage.id} className="animate-fade-in" style={{ animationDelay: `${i * 80}ms`, opacity: 0 }}>
                <div className="glass-card overflow-hidden">
                  <div className="flex items-start gap-4 p-5">
                    <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl font-display text-sm font-bold ${allDone ? "bg-success text-success-foreground" : "bg-primary/10 text-primary"}`}>
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-muted-foreground">{stageCompleted}/{stageLessons.length}</span>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        {stageLessons.map((lesson) => {
                          ...
                          return (
                            <Link key={lesson.id} to={`/lesson/${lesson.id}`} className="flex items-center gap-3 rounded-lg p-2.5 text-sm transition-colors hover:bg-muted/60">
